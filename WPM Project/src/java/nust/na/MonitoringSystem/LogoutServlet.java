...
/**
 * LogoutServlet.java
 * Destroys the session and shows a professional logout confirmation page.
 */
...
public class LogoutServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // Get farmer name before destroying session
        HttpSession session = request.getSession(false);
        String farmerName = "";
        if (session != null) {
            farmerName = (String) session.getAttribute("farmer_name");
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<title>Logged Out - Livestock Monitoring System</title>");
            out.println("<style>");
            ...
            out.println(".box { background: white; padding: 50px 44px; border-radius: 12px; text-align: center; box-shadow: 0 8px 30px rgba(0,0,0,0.12); border-top: 5px solid #2c7a2c; max-width: 440px; width: 100%; }");
            out.println(".icon { font-size: 56px; margin-bottom: 16px; }");
            out.println("h2 { color: #1b5e20; font-size: 22px; margin-bottom: 10px; }");
            out.println("p { color: #555; font-size: 15px; line-height: 1.7; margin-bottom: 20px; }");
            ...
            if (!farmerName.isEmpty()) {
                out.println("<p>Thank you, <strong>" + farmerName + "</strong>.<br/>");
                out.println("You have successfully signed out of the Livestock Monitoring System.</p>");
            } else {
                out.println("<p>You have successfully signed out of the Livestock Monitoring System.</p>");
            }

            out.println("<div class='divider'><hr/><span>or</span><hr/></div>");
            out.println("<a href='login.html' class='btn'>Login Again</a>");
